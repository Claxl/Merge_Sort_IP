...
//------------------------Parameter----------------------
`define TV_IN_left_r "../tv/cdatafile/c.merge_sort.autotvin_left_r.dat"
`define TV_IN_right_r "../tv/cdatafile/c.merge_sort.autotvin_right_r.dat"
`define TV_IN_output_r "../tv/cdatafile/c.merge_sort.autotvin_output_r.dat"
parameter ADDR_WIDTH = 6;
parameter DATA_WIDTH = 32;
parameter left_r_DEPTH = 1;
reg [31 : 0] left_r_OPERATE_DEPTH = 0;
parameter left_r_c_bitwidth = 64;
parameter right_r_DEPTH = 1;
reg [31 : 0] right_r_OPERATE_DEPTH = 0;
parameter right_r_c_bitwidth = 64;
parameter output_r_DEPTH = 1;
reg [31 : 0] output_r_OPERATE_DEPTH = 0;
parameter output_r_c_bitwidth = 64;
parameter START_ADDR = 0;
parameter merge_sort_continue_addr = 0;
parameter merge_sort_auto_start_addr = 0;
parameter left_r_data_in_addr = 16;
parameter right_r_data_in_addr = 28;
...
reg  BREADY_reg = 0;
reg [left_r_c_bitwidth - 1 : 0] mem_left_r [left_r_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_left_r [ (left_r_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * left_r_DEPTH -1 : 0] = '{default : 'hz};
reg left_r_write_data_finish;
reg [right_r_c_bitwidth - 1 : 0] mem_right_r [right_r_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_right_r [ (right_r_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * right_r_DEPTH -1 : 0] = '{default : 'hz};
reg right_r_write_data_finish;
...
//write left_r reg
reg [31 : 0] write_left_r_count = 0;
reg [31 : 0] left_r_diff_count = 0;
reg write_left_r_run_flag = 0;
reg write_one_left_r_data_done = 0;
//write right_r reg
reg [31 : 0] write_right_r_count = 0;
reg [31 : 0] right_r_diff_count = 0;
reg write_right_r_run_flag = 0;
reg write_one_right_r_data_done = 0;
//write output_r reg
...
assign TRAN_control_write_data_finish = 1 & left_r_write_data_finish & right_r_write_data_finish & output_r_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        write_left_r_run_flag <= 0; 
        count_operate_depth_by_bitwidth_and_depth (left_r_c_bitwidth, left_r_DEPTH, left_r_OPERATE_DEPTH);
    end
    else begin
        if (AESL_ready_reg === 1) begin
            write_left_r_run_flag <= 1; 
        end
        else if ((write_one_left_r_data_done == 1 && write_left_r_count == left_r_diff_count - 1) || left_r_diff_count == 0) begin
            write_left_r_run_flag <= 0; 
        end
    end
end
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        write_left_r_count = 0;
    end
    else begin
        if (AESL_ready_reg === 1) begin
            write_left_r_count = 0;
        end
        if (write_one_left_r_data_done === 1) begin
            write_left_r_count = write_left_r_count + 1;
        end
    end
end
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        left_r_write_data_finish <= 0;
    end
    else begin
        if (TRAN_control_start_in === 1) begin
            left_r_write_data_finish <= 0;
        end
        if (write_left_r_run_flag == 1 && write_left_r_count == left_r_diff_count) begin
            left_r_write_data_finish <= 1;
        end
    end
end

initial begin : initial_diff_counter_left_r
    ...
    integer j;
    integer k;
    reg [31 : 0] left_r_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = left_r_c_bitwidth;
    ...
    while (1) begin
        ...
        for (k = 0; k < left_r_OPERATE_DEPTH; k = k + 1) begin
            for (i = 0; i < four_byte_num; i = i + 1) begin
                if (left_r_c_bitwidth < 32) begin
                    left_r_data_tmp_reg = mem_left_r[k];
                end
                else begin
                    for (j = 0; j < 32; j = j + 1) begin
                        if (i*32 + j < left_r_c_bitwidth) begin
                            left_r_data_tmp_reg[j] = mem_left_r[k][i*32 + j];
                        end
                        else begin
                            left_r_data_tmp_reg[j] = 0;
                        end
                    end
                end
                if(image_mem_left_r[k * four_byte_num  + i]!==left_r_data_tmp_reg) begin
                left_r_diff_count = left_r_diff_count + 1;
                end
            end
        end
...
initial begin : write_left_r
    integer write_left_r_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer ceil_align_to_pow_of_two_four_byte_num;
    ...
    while (1) begin
        ...
        for (check_left_r_count = 0; check_left_r_count < left_r_OPERATE_DEPTH; check_left_r_count = check_left_r_count + 1) begin
            if (ongoing_process_number === process_num && process_busy === 0 ) begin
                get_ack = 1;
                if (write_left_r_run_flag === 1 && get_ack === 1) begin
                    process_busy = 1;
                    //write left_r data 
                    for (i = 0; i < four_byte_num; i = i + 1) begin
                        if (left_r_c_bitwidth < 32) begin
                            left_r_data_tmp_reg = mem_left_r[check_left_r_count];
                        end
                        else begin
                            for (j = 0; j < 32; j = j + 1) begin
                                if (i*32 + j < left_r_c_bitwidth) begin
                                    left_r_data_tmp_reg[j] = mem_left_r[check_left_r_count][i*32 + j];
                                end
                                else begin
                                    left_r_data_tmp_reg[j] = 0;
                                end
                        ...
                        if(image_mem_left_r[check_left_r_count * four_byte_num  + i]!==left_r_data_tmp_reg) begin
                        write (left_r_data_in_addr + check_left_r_count * ceil_align_to_pow_of_two_four_byte_num * 4 + i * 4, left_r_data_tmp_reg, write_left_r_resp);
                        write_one_left_r_data_done <= 1;
                        @(posedge clk);
                        write_one_left_r_data_done <= 0;
                        image_mem_left_r[check_left_r_count * four_byte_num + i]=left_r_data_tmp_reg;
                        end
                    end
                    process_busy = 0;
                end   
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        write_right_r_run_flag <= 0; 
        count_operate_depth_by_bitwidth_and_depth (right_r_c_bitwidth, right_r_DEPTH, right_r_OPERATE_DEPTH);
    end
    else begin
        if (AESL_ready_reg === 1) begin
            write_right_r_run_flag <= 1; 
        end
        else if ((write_one_right_r_data_done == 1 && write_right_r_count == right_r_diff_count - 1) || right_r_diff_count == 0) begin
            write_right_r_run_flag <= 0; 
        end
    end
end
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        write_right_r_count = 0;
    end
    else begin
        if (AESL_ready_reg === 1) begin
            write_right_r_count = 0;
        end
        if (write_one_right_r_data_done === 1) begin
            write_right_r_count = write_right_r_count + 1;
        end
    end
end
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        right_r_write_data_finish <= 0;
    end
    else begin
        if (TRAN_control_start_in === 1) begin
            right_r_write_data_finish <= 0;
        end
        if (write_right_r_run_flag == 1 && write_right_r_count == right_r_diff_count) begin
            right_r_write_data_finish <= 1;
        end
    end
end

initial begin : initial_diff_counter_right_r
    integer four_byte_num;
    integer ceil_align_to_pow_of_two_four_byte_num;
    integer c_bitwidth;
    integer i;
    integer j;
    integer k;
    reg [31 : 0] right_r_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = right_r_c_bitwidth;
    count_c_data_four_byte_num_by_bitwidth (c_bitwidth , four_byte_num);
    ceil_align_to_pow_of_two_four_byte_num = ceil_align_to_pow_of_two(four_byte_num);
    while (1) begin
        wait (AESL_ready_reg === 1);
        right_r_diff_count = 0;

        for (k = 0; k < right_r_OPERATE_DEPTH; k = k + 1) begin
            for (i = 0; i < four_byte_num; i = i + 1) begin
                if (right_r_c_bitwidth < 32) begin
                    right_r_data_tmp_reg = mem_right_r[k];
                end
                else begin
                    for (j = 0; j < 32; j = j + 1) begin
                        if (i*32 + j < right_r_c_bitwidth) begin
                            right_r_data_tmp_reg[j] = mem_right_r[k][i*32 + j];
                        end
                        else begin
                            right_r_data_tmp_reg[j] = 0;
                        end
                    end
                end
                if(image_mem_right_r[k * four_byte_num  + i]!==right_r_data_tmp_reg) begin
                right_r_diff_count = right_r_diff_count + 1;
                end
            end
        end
...
initial begin : write_right_r
    integer write_right_r_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer ceil_align_to_pow_of_two_four_byte_num;
    ...
    while (1) begin
        ...
        for (check_right_r_count = 0; check_right_r_count < right_r_OPERATE_DEPTH; check_right_r_count = check_right_r_count + 1) begin
            if (ongoing_process_number === process_num && process_busy === 0 ) begin
                get_ack = 1;
                if (write_right_r_run_flag === 1 && get_ack === 1) begin
                    process_busy = 1;
                    //write right_r data 
                    for (i = 0; i < four_byte_num; i = i + 1) begin
                        if (right_r_c_bitwidth < 32) begin
                            right_r_data_tmp_reg = mem_right_r[check_right_r_count];
                        end
                        else begin
                            for (j = 0; j < 32; j = j + 1) begin
                                if (i*32 + j < right_r_c_bitwidth) begin
                                    right_r_data_tmp_reg[j] = mem_right_r[check_right_r_count][i*32 + j];
                                end
                                else begin
                                    right_r_data_tmp_reg[j] = 0;
                                end
                        ...
                        if(image_mem_right_r[check_right_r_count * four_byte_num  + i]!==right_r_data_tmp_reg) begin
                        write (right_r_data_in_addr + check_right_r_count * ceil_align_to_pow_of_two_four_byte_num * 4 + i * 4, right_r_data_tmp_reg, write_right_r_resp);
                        write_one_right_r_data_done <= 1;
                        @(posedge clk);
                        write_one_right_r_data_done <= 0;
                        image_mem_right_r[check_right_r_count * four_byte_num + i]=right_r_data_tmp_reg;
                        end
                    end
                    process_busy = 0;
                end   
...
// Read data from file 
initial begin : read_left_r_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [151 : 0] token; 
  reg [151 : 0] token_tmp; 
  //reg [left_r_c_bitwidth - 1 : 0] token_tmp; 
  ...
  count_seperate_factor_by_bitwidth (left_r_c_bitwidth , factor);
  fp = $fopen(`TV_IN_left_r ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_left_r); 
      $finish; 
  end 
  read_token(fp, token); 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < left_r_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  ...
                  mem_left_r [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
...
// Read data from file 
initial begin : read_right_r_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [151 : 0] token; 
  reg [151 : 0] token_tmp; 
  //reg [right_r_c_bitwidth - 1 : 0] token_tmp; 
  reg [DATA_WIDTH - 1 : 0] tmp_cache_mem; 
  reg [ 8*5 : 1] str;
    reg [63:0] trans_depth;
  integer transaction_idx; 
  ...
  transaction_idx = 0; 
  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
  count_seperate_factor_by_bitwidth (right_r_c_bitwidth , factor);
  fp = $fopen(`TV_IN_right_r ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_right_r); 
      $finish; 
  end 
  read_token(fp, token); 
  if (token != "[[[runtime]]]") begin             // Illegal format 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < right_r_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
              ...
              if (i%factor == 3) begin
                  tmp_cache_mem [31 : 24] = token_tmp;
                  mem_right_r [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
